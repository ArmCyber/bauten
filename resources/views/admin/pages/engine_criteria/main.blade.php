...
@section('titleSuffix')| <a href="{!! route('admin.engine_criteria.add', ['id'=>$filter->id]) !!}" class="text-cyan"><i class="mdi mdi-plus-box"></i> добавить</a>@endsection
@section('content')
    @if(count($items))
        <div class="card">
            <div class="table-responsive p-2">
                <table class="table table-striped m-b-0 columns-middle">
                    ...
                    <tbody class="table-sortable" data-action="{{ route('admin.engine_criteria.sort') }}">
                    @foreach($items as $item)
                        <tr class="item-row" data-id="{!! $item->id !!}">
                            <td class="item-title">{{ $item->title}}</td>
                            <td>
                                <a href="{{ route('admin.engine_criteria.edit', ['id'=>$item->id]) }}" {!! tooltip('Редактировать') !!} class="icon-btn edit"></a>
    ...
    @slot('title')Удаление критерий@endslot
    <input type="hidden" id="pdf-item-id">
    <p class="font-14">Вы действительно хотите удалить критерий &Lt;<span id="pdm-title"></span>&Gt;?</p>
    @endmodal
@endsection
@push('css')
    ...
    <script>
        ...
        $('#itemDeleteForm').on('submit', function(){
            ...
            if (thisItemId && thisItemId.match(/^[1-9][0-9]{0,9}$/)) {
                ...
                $.ajax({
                    url: '{!! route('admin.engine_criteria.delete') !!}',
                    type: 'post',
                    dataType: 'json',
                    data: {
                        _token: csrf,
