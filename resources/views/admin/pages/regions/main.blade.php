...
@section('titleSuffix')| <a href="{!! route('admin.regions.add', ['id'=>$country->id]) !!}" class="text-cyan"><i class="mdi mdi-plus-box"></i> добавить</a>@endsection
@section('content')
    @if(count($items))
        <div class="card">
            <div class="table-responsive p-2">
                <table class="table table-striped m-b-0 columns-middle">
                    ...
                    <tbody class="table-sortable" data-action="{!! route('admin.regions.sort') !!}">
                    @foreach($items as $item)
                        <tr class="item-row" data-id="{!! $item->id !!}">
                            ...
                            @endif
                            <td>
                                <a href="{{ route('admin.regions.edit', ['id'=>$item->id]) }}" {!! tooltip('Редактировать') !!} class="icon-btn edit"></a>
    ...
    <script>
        ...
        $('#itemDeleteForm').on('submit', function(){
            ...
            if (thisItemId && thisItemId.match(/^[1-9][0-9]{0,9}$/)) {
                ...
                $.ajax({
                    url: '{!! route('admin.regions.delete') !!}',
                    type: 'post',
                    dataType: 'json',
                    data: {
                        _token: csrf,
