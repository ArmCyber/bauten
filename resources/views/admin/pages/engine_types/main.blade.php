...
@section('titleSuffix')| <a href="{!! route('admin.engine_types.add') !!}" class="text-cyan"><i class="mdi mdi-plus-box"></i> добавить</a>@endsection
@section('content')
    @if(count($items))
        <div class="card">
                ...
                <table class="table table-striped m-b-0 columns-middle init-dataTable">
                    ...
                    @foreach($items as $item)
                        <tr class="item-row" data-id="{!! $item->id !!}">
                            ...
                            <td>
                                <a href="{{ route('admin.engine_types.edit', ['id'=>$item->id]) }}" {!! tooltip('Редактировать') !!} class="icon-btn edit"></a>
    ...
    @slot('title')Удаление типа@endslot
    <input type="hidden" id="pdf-item-id">
    <p class="font-14">Вы действительно хотите удалить тип &Lt;<span id="pdm-title"></span>&Gt;?</p>
    @endmodal
@endsection
@push('css')
    ...
    <script>
        ...
        $('#itemDeleteForm').on('submit', function(){
            ...
            if (thisItemId && thisItemId.match(/^[1-9][0-9]{0,9}$/)) {
                ...
                $.ajax({
                    url: '{!! route('admin.engine_types.delete') !!}',
                    type: 'post',
                    dataType: 'json',
                    data: {
                        _token: csrf,
