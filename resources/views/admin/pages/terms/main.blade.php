...
@section('titleSuffix')| <a href="{!! route('admin.terms.add') !!}" class="text-cyan"><i class="mdi mdi-plus-box"></i> добавить</a>@endsection
@section('content')
    @if(count($items))
        <div class="card">
            <div class="table-responsive p-2">
                <table class="table table-striped m-b-0 columns-middle">
                    ...
                    <tbody class="table-sortable" data-action="{{ route('admin.terms.sort') }}">
                    @foreach($items as $item)
                        <tr class="item-row" data-id="{!! $item->id !!}">
                            ...
                            @endif
                            <td>
                                <a href="{{ route('admin.terms.edit', ['id'=>$item->id]) }}" {!! tooltip('Редактировать') !!} class="icon-btn edit"></a>
    ...
    @slot('title')Удаление блока условии@endslot
    <input type="hidden" id="pdf-item-id">
    <p class="font-14">Вы действительно хотите удалить блок &Lt;<span id="pdm-title"></span>&Gt;?</p>
    @endmodal
@endsection
@push('js')
    <script>
                ...
                $.ajax({
                    ...
                    success: function(e){
                        if (e.success) {
                            loader.removeClass('shown');
                            blocked = false;
                            toastr.success('Блок условии удален.');
                            modal.modal('hide');
