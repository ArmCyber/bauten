...
@section('titleSuffix')| <a href="{!! route('admin.delivery_regions.add') !!}" class="text-cyan"><i class="mdi mdi-plus-box"></i> добавить</a>@endsection
@section('content')
    @if(count($items))
        <div class="card">
                ...
                <table class="table table-striped m-b-0 columns-middle init-dataTable">
                    ...
                    @foreach($items as $item)
                        <tr class="item-row" data-id="{!! $item->id !!}">
                            ...
                            <td><a href="{{ route('admin.delivery_cities.main', ['id'=>$item->id]) }}">Перейти к населенным пунктам ({!! $item->cities_count !!})</a></td>
                            <td>
                                <a href="{{ route('admin.delivery_regions.edit', ['id'=>$item->id]) }}" {!! tooltip('Редактировать') !!} class="icon-btn edit"></a>
