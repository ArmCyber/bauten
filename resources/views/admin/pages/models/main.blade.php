...
@section('titleSuffix')| <a href="{!! route('admin.models.add', ['id'=>$mark->id]) !!}" class="text-cyan"><i class="mdi mdi-plus-box"></i> добавить</a>@endsection
@section('content')
    @if(count($items))
        <div class="card">
            <div class="table-responsive p-2">
                <table class="table table-striped m-b-0 columns-middle init-dataTable">
                    ...
                    <tbody class="table-sortable" data-action="{!! route('admin.models.sort') !!}">
                    @foreach($items as $item)
                        <tr class="item-row" data-id="{!! $item->id !!}">
                            ...
                            @endif
                            <td><a href="{{ route('admin.generations.main', ['id'=>$item->id]) }}">Перейти к поколениям ({!! $item->generations_count !!})</a></td>
                            <td>
    ...
    @slot('title')Удаление модели@endslot
    <input type="hidden" id="pdf-item-id">
    <p class="font-14">Вы действительно хотите удалить модель &Lt;<span id="pdm-title"></span>&Gt;?</p>
    @endmodal
@endsection
@push('css')
